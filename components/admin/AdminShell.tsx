    ...
    <div className="app-shell admin-shell">
      <aside className="sidebar admin-sidebar">
        ...
        <nav className="main-nav" aria-label="Admin navigation">
          {navigation.map((item) => <Link key={item.href} href={item.href} className={pathname === item.href || pathname.startsWith(`${item.href}/`) ? "nav-item active" : "nav-item"}><span className="nav-icon">{item.icon}</span>{item.label}</Link>)}
        </nav>
        <div className="sidebar-bottom">
          <Link className="nav-item" href="/live"><span className="nav-icon">◉</span>Public live</Link>
          ...
          <button className="nav-item admin-logout" type="button" onClick={() => { void fetch("/api/auth/logout", { method: "POST" }).finally(() => router.replace("/admin/login")); }}><span className="nav-icon">↪</span>Sign out</button>
        </div>
      </aside>
      <main className="main-content">
        <header className="topbar">
          <div className="crumb">2026 SEASON <span>/</span> AUCTION CONTROL ROOM</div>
          <div className="top-actions"><Link className="live-link" href="/live">◉ View public display</Link><button className="icon-button" aria-label="Notifications">♧</button><button className="icon-button" aria-label="Help">?</button></div>
        </header>
        {children}
      </main>
    </div>
