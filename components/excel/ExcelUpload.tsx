...
function makePlayerCode(name: string, address: string, phone: string) {
  const source = `${name}-${address || phone}`
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "");
...
function toImportRow(row: Record<string, unknown>) {
  ...
  if (ageValue && (age === undefined || age < 1 || age > 120))
    errors.push("Age must be a valid number");
  if (genderValue && !normalizePlayerGender(genderValue))
  ...
  else if (toNumber(basePriceValue) === undefined)
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const response = await fetch("/api/players/import", {
        ...
        body: JSON.stringify({
          players: validRows.map((item) => item.player!),
        }),
      });
      ...
      if (!response.ok)
        throw new Error(result.error ?? "Unable to import players.");
      ...
      setSummary({
        ...
        warnings:
          parsed.reduce((count, item) => count + item.warnings.length, 0) +
          duplicateRows,
    ...
    <section className="panel import-panel">
      ...
      <div className="upload-card">
        ...
        <div className="upload-actions">
          ...
          >
            {processing && (
              <span className="import-loader" aria-hidden="true" />
            )}
      ...
      <div className="excel-table-wrap">
        ...
        {previewRows.map((row) => (
          <div className="excel-table-row" key={row[0]}>
            {row.map((value, index) => (
              <span key={`${row[0]}-${importColumns[index]}`}>{value}</span>
            ))}
          </div>
        ))}
      </div>
